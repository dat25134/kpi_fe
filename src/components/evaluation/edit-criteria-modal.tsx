...
import { useUpdateCriteria } from "@/hooks/useCriteria";
...
const EditCriteriaModal: React.FC<EditCriteriaModalProps> = ({ open, onOpenChange, onSuccess, criteria }) => {
  ...
  const { trigger, isMutating } = useUpdateCriteria();
  ...
  useEffect(() => {
    if (criteria) {
      setForm({
        ...
        max_score: Number(criteria.max_score) || 1,
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex items-center gap-2 mt-6">
                <input
                  ...
                  id="is_active_edit"
                />
                <label htmlFor="is_active_edit" className="text-sm">Kích hoạt</label>
              </div>
            </div>
          </div>
          <DialogFooter>
